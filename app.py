...
def safe_parse_xml(xml_bytes: bytes) -> ET.Element:
    ...
    return ET.fromstring(xml_str.encode("utf-8", errors="replace"))


def choose_files_dialog(window) -> list:
    if not window:
        return []
    try:
        dialog_type = getattr(
...
class FlattenedFTViewCompiler:
    def __init__(self, xml_bytes: bytes, file_name: str, tag_overrides: dict = None):
        ...
        self.root = safe_parse_xml(xml_bytes)
        self.tag_overrides = tag_overrides or {}

        settings = self.root.find("./displaySettings") or self.root.find(".//displaySettings")
    ...
    def open_import_dialog(self):
        ...
        def worker():
            try:
                ...
                print(f"Worker thread error: {worker_err}")
            finally:
